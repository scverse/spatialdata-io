...
def xenium_explorer_selection(path: str | Path, pixelsize: float = 0.2125) -> Polygon:
    """Read the coordinates of a selection `.csv` file exported from the [Xenium Explorer](https://www.10xgenomics.com/support/software/xenium-explorer/latest).
    ...
    If [`spatialdata_xenium_explorer`](https://github.com/quentinblampey/spatialdata_xenium_explorer) was used,
    the `pixelsize` argument must be set to the one used during conversion with `spatialdata_xenium_explorer`.
