...
@dataclass
class _CellsZarr:
    """Wraps cells.zarr.zip, pre-computing shared state once.

    Centralizes all zarr-based logic: cell_id string encoding, indices mappings
    (polygon_sets for v2.0+, seg_mask_value for v1.3.x), labels rasters, and
    cell summary metadata. Version-specific branching happens here so that
    callers can use simple feature-detection booleans.
    """
    ...
    def get_indices_mapping(self, mask_index: int) -> pd.DataFrame | None:
        ...
        For v1.3.x (seg_mask_value): cells only (mask_index=1); nuclei return None.
        For v < 1.3.0: returns None (no mapping available).
        """
        if self.cell_id_str is not None and self.has_polygon_sets:
            return _get_indices_mapping_from_zarr(self.group, self.cell_id_str, mask_index)
    ...
    def get_labels(self, mask_index: int, labels_models_kwargs: Mapping[str, Any] = MappingProxyType({})) -> DataArray:
        """Read the labels raster from masks/{mask_index}."""
        return _get_labels(self.group, mask_index, labels_models_kwargs)

    def get_cell_summary(self) -> pd.DataFrame:
        """Read cells summary table (z_level, nucleus_count, etc.)."""
        return _get_cells_metadata_table_from_zarr(self.group, self.cell_id_str)
...
def xenium(
    ...
    *,
    ...
    cells_zarr_ctx = _CellsZarr.open(path, version) if need_cells_zarr else None
    ...
    if nucleus_labels:
        labels["nucleus_labels"] = cells_zarr_ctx.get_labels(0, labels_models_kwargs)
    if cells_labels:
        labels["cell_labels"] = cells_zarr_ctx.get_labels(1, labels_models_kwargs)
    ...
    if table is not None:
        _enrich_table(table, cells_zarr_ctx, version, cell_indices_mapping if cells_labels else None, cells_as_circles)

    # --- assemble SpatialData ---
    tables = {"table": table} if table is not None else {}
    shapes = polygons
    if cells_as_circles:
        shapes[specs["region"]] = circles
...
def _get_polygons(
    ...
    if indices_mapping is not None and not has_label_id and is_nucleus:
        # xenium 2.0.0 early builds: parquet lacks label_id and groups nucleus boundaries by cell_id,
        # merging multiple nuclei of multinucleate cells into one polygon. The zarr indices_mapping has
        # one entry per nucleus, so the cardinalities don't match. Fall through to the parquet fallback.
    ...
    if indices_mapping is not None:
        ...
        # Fallback for old Xenium versions (< 1.3.0, or 1.3.x without polygon_sets in the zarr).
        # nucleus_indices_mapping is never populated for legacy formats, so nucleus boundaries also
        # reach this path. To keep the index type consistent across versions (int for nuclei, str
        # for cells), assign a 1-based integer index for nuclei and use the parquet cell_id str for cells.
        unique_ids = id_col.filter(np.concatenate([[True], change_mask])).to_pylist()
        if is_nucleus:
            # Without label_id or indices_mapping, we can't recover the exact raster label indices.
            # Use 1-based sequential ints; this covers:
            # - Xenium < 1.3.0 (no nucleus raster exists, so no alignment needed)
            # - Xenium 2.0.0 early builds without label_id (parquet merges multinucleate cells,
            #   so the parquet and raster have different cardinalities and can't be aligned anyway)
            index = np.arange(1, len(unique_ids) + 1, dtype=np.int64)
        else:
            index = _decode_cell_id_column(pd.Series(unique_ids)).values
        geo_df = GeoDataFrame({"geometry": geoms}, index=index)
...
def _enrich_table(
    table: AnnData,
    cells_zarr_ctx: _CellsZarr | None,
    version: packaging.version.Version | None,
    ...
    if cells_zarr_ctx is not None and version is not None and version >= packaging.version.parse("2.0.0"):
