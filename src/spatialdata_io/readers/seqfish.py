...
def seqfish(
    ...
    images = {
        f"label_{x}": Image2DModel.parse(imread(path / dapi_file[x], dims=("c", "y", "x")), **imread_kwargs)
        for x in range(1, _sections + 2)
    }
    labels = {
        f"image_{x}": Labels2DModel.parse(imread(path / cell_mask_file[x]).squeeze(), dims=("y", "x"), **imread_kwargs)
        for x in range(1, _sections + 2)
    }
    points = {
        f"transcripts_{x}": PointsModel.parse(
            pd.read_csv(path / transcript_file[x], delimiter=","),
