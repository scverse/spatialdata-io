...
from ome_types.model.simple_types import ChannelID, Color, ImageID, PixelsID
...
parser.add_argument("-o", "--output", required=True, help="Directory to save the stack.")

parser.add_argument("-c", "--cycle", required=True, type=int, help="Number of the antigen cycle.")
...
def create_ome(img_info, xy_tile_positions_units, img_path):
    ...
    for ch in range(0, no_of_channels):
        tiff_block.append(TiffData(first_c=ch, ifd=ch, plane_count=1, uuid=unique_identifier))
    ...
    img_block = []
    for t in range(0, no_of_tiles):
        img_block.append(Image(id=ImageID(f"Image:{t}"), pixels=pix_block[t]))
