# Contributing to spatialdata-io

This guide describes how to contribute a **reader** or a **converter** to `spatialdata-io`, and what we need
in order to review it efficiently. It is intended for:
...
Feedback on this document is highly welcomed: you are encouraged to open a PR to improve this guide.
**A summary checklist is provided at the end of this document.**

:::{admonition} Looking for the development setup instead?
:class: seealso

This page covers only what is specific to `spatialdata-io`. The parts that are not repository-specific are
documented elsewhere:

- **Generic scverse tooling** — setting up a development environment, code style and pre-commit hooks, running
  the tests, building the documentation, publishing a release: see the {doc}`generic contributing guide
  <contributing>`. That page is shared across scverse packages and is kept in sync with the
  [scverse cookiecutter template](https://github.com/scverse/cookiecutter-scverse), so it contains no
  `spatialdata-io` specifics.
- **The SpatialData technical stack** — `spatialdata-io` inherits it, so please also read the
  [contribution guide of the `spatialdata` repository](https://github.com/scverse/spatialdata/blob/main/docs/contributing.md).
